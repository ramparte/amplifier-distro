...
_STATIC_DIR = Path(__file__).parent.parent / "src" / "amplifier_distro" / "server" / "apps" / "voice" / "static"
...
class TestIndexFileExists:
    ...
    def test_index_html_is_not_empty(self) -> None:
        assert INDEX_HTML.stat().st_size > 500, "index.html appears to be empty or too small"
...
class TestIndexContent:
    ...
    def test_disconnect_function_declared(self) -> None:
        assert "disconnect" in self.html, "useWebRTC must expose a disconnect() function"
    ...
    def test_creates_rtc_peer_connection(self) -> None:
        assert "RTCPeerConnection" in self.html, "connect() must create an RTCPeerConnection"

    def test_uses_stun_server(self) -> None:
        assert "stun:stun.l.google.com" in self.html, "RTCPeerConnection must use Google STUN"
    ...
    def test_creates_data_channel(self) -> None:
        assert "oai-events" in self.html, "connect() must create data channel named 'oai-events'"
