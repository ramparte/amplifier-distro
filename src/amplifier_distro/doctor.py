...
def _amplifier_home(override: Path | None = None) -> Path:
    """Return the resolved amplifier home directory."""
    if override is not None:
        return override
    return Path(conventions.AMPLIFIER_HOME).expanduser()
...
def _check_config_exists(home: Path) -> DiagnosticCheck:
    """Check that distro.yaml exists and is valid YAML."""
    cfg_path = home / conventions.DISTRO_CONFIG_FILENAME
    if not cfg_path.exists():
        return DiagnosticCheck(
            name="Config file",
            status=CheckStatus.error,
            message=f"distro.yaml not found at {cfg_path}",
            fix_available=False,
...
def _check_keys_permissions(home: Path) -> DiagnosticCheck:
    """Check that keys.yaml has mode 600 (owner-only) on Unix."""
    keys_path = home / conventions.KEYS_FILENAME
    if not keys_path.exists():
        return DiagnosticCheck(
            name="Keys permissions",
            status=CheckStatus.ok,
            message="keys.yaml not present (nothing to check)",
        )
    if not _is_unix():
        return DiagnosticCheck(
            name="Keys permissions",
    ...
    if mode == 0o600:
        return DiagnosticCheck(
            name="Keys permissions",
            status=CheckStatus.ok,
            message="keys.yaml has correct permissions (600)",
        )
    return DiagnosticCheck(
        name="Keys permissions",
        status=CheckStatus.warning,
        message=f"keys.yaml has mode {oct(mode)}, should be 0o600",
        fix_available=True,
        fix_description="Set keys.yaml permissions to 600",
    )
...
def _check_slack_configured(home: Path) -> DiagnosticCheck:
        ...
        try:
            ...
            if data.get("SLACK_BOT_TOKEN"):
                return DiagnosticCheck(
                    name="Slack bridge",
                    status=CheckStatus.ok,
                    message="SLACK_BOT_TOKEN found in keys.yaml",
                )
    ...
    return DiagnosticCheck(
        name="Slack bridge",
        status=CheckStatus.warning,
        message="SLACK_BOT_TOKEN not found in env or keys.yaml",
    )
        ...
        try:
            ...
            if data.get("OPENAI_API_KEY"):
                return DiagnosticCheck(
                    name="Voice config",
                    status=CheckStatus.ok,
                    message="OPENAI_API_KEY found in keys.yaml",
                )
        except yaml.YAMLError:
            pass
    return DiagnosticCheck(
        name="Voice config",
        status=CheckStatus.warning,
        message="OPENAI_API_KEY not found in env or keys.yaml",
    )
...
def _check_identity() -> DiagnosticCheck:
    """Check that a GitHub identity is configured in distro.yaml."""
    config = load_config()
    if config.identity.github_handle:
        return DiagnosticCheck(
            name="Identity",
            status=CheckStatus.ok,
            message=f"@{config.identity.github_handle}",
        )
    return DiagnosticCheck(
        name="Identity",
        status=CheckStatus.error,
...
def run_diagnostics(amplifier_home: Path) -> DoctorReport:
    ...
    for check in report.checks:
        ...
        if check.name == "Memory directory":
            ...
            if keys_path.exists() and _is_unix():
                ...
                fixed.append("Set keys.yaml permissions to 600")
