...
class LocalBridge:
    ...
    def _resolve_distro_bundle(bundle_name_override: str | None) -> str:
        ...
        2. ``bundle.active`` from distro.yaml config (user's explicit choice)
        3. Convention-path file: ``~/.amplifier/bundles/distro.yaml``
        4. RuntimeError if nothing found
        """
        if bundle_name_override:
            return bundle_name_override

        # User's explicit bundle choice from distro.yaml config
        ...
        try:
            ...
            if active:
                logger.info(
                    "Using bundle.active=%r from distro.yaml config",
                    active,
                )
                return active
        except Exception:  # noqa: BLE001
            logger.debug("Could not load distro config for bundle resolution")
        ...
        raise RuntimeError(
            "No bundle.active configured in distro.yaml and no "
            f"convention bundle found at {path}. "
